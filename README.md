...
video ──► frames ──► camera tiles + masks ──► COLMAP ──► Brush ──► splat
          (thin)     (v360, one decode)       (rig)      (3DGS)
```

The extraction engine is ffmpeg's `v360` filter. 360extract decodes the source **once** and
fans it out to every camera in a single pass:
...
Most machines have several ffmpeg builds and the first on `PATH` is often an old one.
360extract probes every candidate and picks the newest usable build rather than trusting
`PATH` order:
...
`360extract project show dataset/` reports the settings and which stages are done or stale.
...
Lists every ffmpeg it found, marks the one it would use, and says why the others were
rejected. Then the same for COLMAP, including whether that build has `rig_configurator` —
without it, reconstruction cannot use the rig and the whole approach falls back to ordinary
...
A source outside the project folder is stored as an absolute path; one inside stays relative,
so a dataset carrying its own footage is still movable.
...
Reads every setting from `project.json` and skips stages whose fingerprint still matches.
This is the command to reach for; `extract` and `mask` below are the project-less versions.
...
[Output layout](#output-layout)), `run_colmap.sh`, and with `--gpx` a
`geo_registration.txt`.

A GPX track is worth supplying: it geo-registers the model through `model_aligner`, and that
similarity transform carries a **uniform scale** — the only thing that makes a cleanup radius
mean metres rather than arbitrary units.
...
virtualenv on first run, checks ffmpeg, and opens `127.0.0.1:8360`.

Four stages, in order:

| Stage | What happens |
|---|---|
| **Start** | pick a video or project, choose sampling and masking, split into segments, **Process** |
| **Capture** | the rig on the panorama — aim cameras, set FOV, preview masks, **Generate cameras** |
| **Reconstruct** | COLMAP, with the sparse cloud drawn as it is built |
| **Train** | Brush, with the splat loaded as each export lands, plus the splat cleanup |
...
footprint to aim it. The mask overlay tints what will be excluded, on the panorama itself.

That overlay is not decoration: `tests/test_overlay_geometry.py` runs the shipped
`geometry.js` under node, places markers just inside and just outside each drawn edge, and
checks them against real ffmpeg extractions. A UI that draws coverage it does not have is
worse than no UI, because it hides exactly the occluder you were trying to exclude.

Node is only needed for those tests, never to use the tool.

## Projects

A project is one `project.json` at the root of the dataset it describes. That makes the folder
self-describing: move it, hand it to someone else, come back in a month, and the settings
arrive with the pixels.
...
Fingerprints are per stage, so changing the detector does not force a re-extract while
changing the rig does. Redoing a stage clears the ones after it, which would otherwise claim
to be current while describing images that no longer exist.
...
`--sharp` states a **window in seconds**, not a rate: `1` keeps one frame per second, `0.5`
one per half second. Smaller means more frames.

**It is usually the better choice.** Uniform sampling is blind to motion blur — it takes
whatever frame the tick lands on, and on a walking or driving capture a good share of those
are smeared. Blurred frames are worse than useless: they contribute no matchable features and
drag the reconstruction down.

Sharpness comes from ffmpeg's own `blurdetect` filter, so there is no extra dependency. It
costs one analysis decode before extraction, and reports what it did:
...
Flat or dark footage can be corrected before it is cut into tiles:

```bash
360extract extract CLIP.mp4 --rig ring --exposure 0.7 --contrast 1.1
```
...
same way come out byte-identical. The default grade is the identity and emits no filter at
all, so an ungraded extraction is byte-for-byte what it was before this feature existed.
...
Or press **Auto** in Capture, which grades the frame you are looking at and prints what it
measured.

This grades **for a splat trainer, not for a viewer**, and those want different things. 3DGS
fits what it is given: a clipped highlight has no gradient to descend, so gaussians land on it
flat and white; a crushed shadow takes the features COLMAP was going to match with; boosted
chroma is 4:2:0 subsampling baked into per-gaussian colour. Three ideas follow.
...
pixels that will be thrown away — a bright overcast drags the median up and darkens the street
to compensate. Only the band between −60° and +40° elevation is measured. On one real drive
that moved the median from 0.27 (whole frame) to 0.20 (street level).

**Simulate before committing.** Every correction is applied to the measured pixels and checked
for clipping, then backed off until it stops, with a 1% budget at each end. Sweeping that
budget showed 1% is the knee: going to 4% bought about 0.2 stops and took clipped pixels from
~0.5% to 3%.

**Never add saturation.** It is a ceiling, not a target: it comes down for a lurid camera
profile and is otherwise exactly 1.0. Black is only pulled when the picture is both lifted and
...
and asks for a small proxy while a slider is moving, then the full-size frame when released.
...
inflates the dataset without adding a single real feature to match. Sizing is per-camera, so a
45° camera in a mixed rig is not padded out to match a 90° one. `--width`/`--height` override
with a fixed size for every camera.
...
cone — a hood, a mount arm, a wing mirror — paint it directly onto the panorama in the app.
...
These move, so no painted region catches them.
...
few pixels (`--dilate`, default 6) because segmentation edges sit slightly inside the object,
and a sliver of leftover pedestrian is enough to seed a floater.
...
`pip install torch` gives a CPU build on Windows. For CUDA:
...
`00001.jpg` inside `images/<camera>/images/` rather than carrying the camera name; and the rig
must be configured *before* matching, because sequential matching pairs images by frame.

Verified against real COLMAP rather than against our reading of the format:
`rig_configurator` accepts the file, produces **one rig containing every camera**, gathers
**all cameras into each frame**, and adopts our exact intrinsics with `prior_focal_length = 1`.
Those tests run whenever COLMAP is installed and skip when it is not — `pytest -m colmap`.
...
without writing. The same thing is in the Train tab, with a Result / Removed toggle on the
viewer.
...
**Filenames stay identical across camera folders**, and that is load-bearing:
`rig_configurator` groups images into *frames* by what is left of the path once a camera's
`image_prefix` is stripped, so the prefixes are `fwd/images/` and the frame is `00001.jpg`. Put
the camera in the filename and every image becomes its own frame, dissolving the rig constraint
that keeps a panoramic tile set from drifting. (COLMAP 4.0 has no `image_suffix` to strip it
back off.) See `dataset.py`.
...
older shape. Older datasets — a repeated clip folder, or images directly in
`images/<camera>/` — still open; lookups follow whatever is on disk.
